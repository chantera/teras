...
_dataset_root = os.environ.get('TERAS_DATASET_ROOT',
                               os.path.expanduser('~/.teras/dataset'))
...
def set_dataset_root(path):
    ...
    environment variable ``TERAS_DATASET_ROOT``. The other is by using this
    function. If both are specified, one specified via this function is used.
    The default dataset root is ``$HOME/.teras/dataset``.
