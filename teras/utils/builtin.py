...
def patch(klass, attr, value, hide_from_dir=False):
    """Apply a patch a built-in `klass` with `attr` set to `value`
    ...
    It's possible to attach methods as class methods, just do the following:
      ...
      ...     return cls(1.5)
      >>> patch(float, "myclassmethod", classmethod(myclassmethod))
      >>> float.myclassmethod()
      1.5
...
def tear(klass, attr):
    """Tear a patch from a built-in object
